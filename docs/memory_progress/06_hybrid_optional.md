...
- 向量化：worker 的 `embed_facts` 作业按 scope + fingerprint 回填，批量 `AI_MEMORY_EMBEDDING_BATCH_SIZE`、日预算 `AI_MEMORY_EMBEDDING_DAILY_BUDGET`；`put_embedding` 每个事实只保留当前 fingerprint 的一行，`prune_stale_embeddings` 清掉 superseded / revision 不符的行。
