...
DeepSeek remains for its non-chat task APIs, including calendar rendering, email, notice, material indexing, and embeddings. Legacy private implementation helpers that are no longer reachable are retained temporarily only where their removal would require a broad rewrite of `aisvc.py`; they have no public route or configuration source.
